...
public final class MultiAgentContractVerifier {
    ...
    private static void verifyServiceImplementation(List<String> violations) {
        try {
            ...
            Class<?> expectedInterface = Class.forName("com.shreeai.os.platform.kernels.multiagent.api.MultiAgentService");
    ...
    private static void verifyEngineContract(List<String> violations) {
        try {
            Class<?> engineInterface = Class.forName("com.shreeai.os.platform.kernels.multiagent.engine.MultiAgentProcessingEngine");
            if (!engineInterface.isInterface()) {
                violations.add("MultiAgentProcessingEngine in engine package is not an interface");
            }

            Class<?> defaultEngine = Class.forName("com.shreeai.os.platform.kernels.multiagent.engine.DefaultMultiAgentProcessingEngine");
    ...
    private static void verifyCanonicalModelUsage(List<String> violations) {
        try {
            Class<?> serviceClass = Class.forName("com.shreeai.os.platform.kernels.multiagent.api.MultiAgentService");
