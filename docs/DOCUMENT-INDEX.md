# Document Index

## Document Information

| Field              | Value              |
|--------------------|--------------------|
| **Document ID**    | DOC-INDEX-001      |
| **Document Type**  | Document Catalog   |
| **Platform**       | Shree AI OS        |
| **Version**        | 1.0                |
| **Status**         | Active             |
| **Owner**          | Chief AI Architect |
| **Founder**        | Darshan Rathod     |
| **Classification** | Platform Knowledge |
| **Created**        | 11 July 2026       |
| **Last Updated**   | 11 July 2026       |

---

## Purpose

This document provides a searchable catalog of every official document in the Shree AI OS documentation repository.

Documents are organized by category for easy navigation.

---

## Foundational Documents

| ID | Title | Location | Status |
|----|-------|----------|--------|
| CONST-001 | Constitution of Shree AI OS | [constitution/CONST-001-CONSTITUTION-OF-SHREE-AI-OS.md](governance/constitution/CONST-001-CONSTITUTION-OF-SHREE-AI-OS.md) | Draft |
| VISION-001 | Platform Vision | [vision/VISION-001-PLATFORM-VISION.md](philosophy/vision/VISION-001-PLATFORM-VISION.md) | Draft |
| MISSION-001 | Platform Mission | [mission/MISSION-001-PLATFORM-MISSION.md](philosophy/mission/MISSION-001-PLATFORM-MISSION.md) | Draft |
| ORG-001 | Engineering Organization | [organization/ORG-001-ENGINEERING-ORGANIZATION.md](governance/organization/ORG-001-ENGINEERING-ORGANIZATION.md) | Draft |
| WORKFLOW-001 | Engineering Workflow | [workflow/WORKFLOW-001-ENGINEERING-WORKFLOW.md](workflow/WORKFLOW-001-ENGINEERING-WORKFLOW.md) | Draft |
| RULE-001 | Platform Engineering Rules | [rules/RULE-001-PLATFORM-ENGINEERING-RULES.md](governance/rules/RULE-001-PLATFORM-ENGINEERING-RULES.md) | Draft |

---

## Architecture Documents

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-ARCH-001 | Architecture Documentation Index | [architecture/README.md](architecture/README.md) | Active |
| DOC-ARCH-RUNTIME-001 | Runtime Architecture Index | [architecture/runtime/README.md](architecture/platform/runtime/README.md) | Active |

**Note:** Architecture Design Documents (ADD) will be added as they are created.

---

## Architecture Decision Records (ADR)

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-ADR-001 | ADR Documentation Index | [ADR/README.md](ADR/README.md) | Active |

**Note:** Individual ADR documents are stored in the [ADR](ADR/) directory. See the ADR README for a complete listing.

---

## Specifications

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-SPEC-001 | Technical Specifications Index | [specifications/README.md](specifications/README.md) | Active |

**Note:** Individual specification documents will be added as they are created.

---

## Reports

| ID | Title | Location | Status |
|----|-------|----------|--------|

**Note:** Reports and analysis documents are stored in the [ADR](ADR/) directory.

---

## Research

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-RESEARCH-001 | Research Documentation Index | [research/README.md](research/README.md) | Active |

**Note:** Individual research documents will be added as they are created.

---

## Roadmaps

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-ROADMAP-001 | Platform Roadmap Index | [roadmap/README.md](roadmap/README.md) | Active |

**Note:** Individual roadmap documents will be added as they are created.

---

## Transition Documents

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-TRANS-001 | Transition Guides Index | [transition/README.md](transition/README.md) | Active |

**Note:** Individual transition documents will be added as they are created.

---

## Standards Documents

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-STD-001 | Engineering Standards Index | [standards/README.md](engineering/standards/README.md) | Active |

---

## Doctrine Documents

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-DOCTRINE-001 | Platform Doctrine Index | [doctrine/README.md](philosophy/doctrine/README.md) | Active |

---

## Philosophy Documents

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-PHIL-001 | Engineering Philosophy Index | [philosophy/README.md](philosophy/README.md) | Active |

---

## Decision Records

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-DEC-001 | Platform Decisions Index | [decisions/README.md](decisions/README.md) | Active |

---

## Handbook

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-HB-001 | Contributor Handbook Index | [handbook/README.md](handbook/README.md) | Active |

---

## Journal

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-JOURNAL-001 | Development Journal Index | [journal/README.md](journal/README.md) | Active |

---

## Vision Documents

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-VISION-001 | Vision Documentation Index | [vision/README.md](philosophy/vision/README.md) | Active |
| VISION-001 | Platform Vision | [vision/VISION-001-PLATFORM-VISION.md](philosophy/vision/VISION-001-PLATFORM-VISION.md) | Draft |

---

## Organization Documents

| ID | Title | Location | Status |
|----|-------|----------|--------|
| DOC-ORG-001 | Organization Documentation Index | [organization/README.md](governance/organization/README.md) | Active |
| ORG-001 | Engineering Organization | [organization/ORG-001-ENGINEERING-ORGANIZATION.md](governance/organization/ORG-001-ENGINEERING-ORGANIZATION.md) | Draft |

---

## Navigation

### Quick Links

- [Documentation Home](README.md)
- [Foundation Layer](foundation/README.md)
- [Constitution](governance/constitution/CONST-001-CONSTITUTION-OF-SHREE-AI-OS.md)
- [Architecture](architecture/README.md)
- [Architecture Decision Records](ADR/README.md)
- [Engineering Standards](engineering/standards/README.md)

---

## Constitutional Authority

This document derives authority from **CONST-001 — Constitution of Shree AI OS**.

---

**Platform:** Shree AI OS  
**Maintained By:** Chief AI Architect  
**Constitutional Authority:** CONST-001

**End of Document Index**
